...
class ExerciseControllerTest extends TestCase
{
    ...
    public function tast_update_exercise()
    {
        $item = Exercise::factory()->create();
        $data = [
            'name' => 'exercise_update',
            'email' => 'exercise@update_example.com',
            'profile' => 'exercise',
        ];
        $response = $this->post('/api/v1/exercise/' . $item->id, $data);
        $response->assertStatus(200);
        $this->assertDatabaseHas('exercises', $data);
    }
    public function tast_destroy_exercise()
    {
        $item = Exercise::factory()->crete();
        $response = $this->dalete('/api/v1/exercise' . $item->id);
        $response->assertStatus(200);
